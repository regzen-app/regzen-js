...
export default class RegzenClient {
  constructor({ applicationId, apiBaseUrl = 'https://api.regzen.com/api' }) {
  ...
  _signIn() {
    ...
    this._popupWindow = window.open(
      `${this._apiBaseUrl}/oauth?application_id=${this._applicationId}`,
      '_blank',
  ...
  _poll() {
    ...
    this._statusPoll = setInterval(() => {
      this._request.open('POST', `${this._apiBaseUrl}/oauth/status`);
