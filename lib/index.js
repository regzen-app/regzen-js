...
export default class RegzenClient {
  constructor({ applicationId, apiBaseUrl = 'https://api.regzen.com' }) {
  ...
  _signIn() {
    ...
    this._popupWindow = window.open(
      `${this._apiBaseUrl}/api/oauth?application_id=${this._applicationId}`,
      '_blank',
  ...
  _poll() {
    ...
    this._statusPoll = setInterval(() => {
      this._request.open('POST', `${this._apiBaseUrl}/api/oauth/status`);
      this._request.setRequestHeader('Content-Type', 'application/json');
      this._request.send(JSON.stringify({ attempt: this._attempt }));
    }, 2000);
  }
  ...
  _receiveMessage(message) {
    if (message.origin !== this._apiBaseUrl) {
      return;
    }
  ...
  _injectFontsAndStyles() {
    ...
    styles.href = 'https://unpkg.com/regzen-client/dist/index.css';
  ...
  _createSignInButton() {
    ...
    if (signInButton) {
      ...
      div.appendChild(document.createTextNode('Sign in with Regzen'));
